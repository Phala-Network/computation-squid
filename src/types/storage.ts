...
export class IdentityIdentityOfStorage {
   ...
   *  TWOX-NOTE: OK ― `AccountId` is a secure hash.
   */
  async getAsV1191(key: Uint8Array): Promise<v1191.Registration | undefined> {
    assert(this.isV1191)
    return this._chain.getStorage(this.blockHash, 'Identity', 'IdentityOf', key)
  }

  async getManyAsV1191(keys: Uint8Array[]): Promise<(v1191.Registration | undefined)[]> {
    assert(this.isV1191)
    return this._chain.queryStorage(this.blockHash, 'Identity', 'IdentityOf', keys.map(k => [k]))
  }

  async getAllAsV1191(): Promise<(v1191.Registration)[]> {
    assert(this.isV1191)
...
export class PhalaComputationTokenomicParametersStorage {
   ...
   *  Tokenomic parameters used by Gatekeepers to compute the V promote.
   */
  async getAsV1191(): Promise<v1191.TokenomicParameters | undefined> {
    assert(this.isV1191)
